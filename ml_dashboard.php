...
$(document).ready(function() {
  ...
  function renderTable(couples) {
    ...
    couples.forEach(couple => {
      ...
      const generatedAt = analysis?.generated_at ? 
        new Date(analysis.generated_at).toLocaleString() : 'Not analyzed';
  ...
  $('#trainModelsBtn').on('click', async function(){
    const btn = $(this);
    btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin mr-1"></i>Training...');
    
    try {
      const response = await fetch('./ml_api.php?action=train', { method: 'POST' });
      ...
      if (data.status === 'success') {
        if (typeof Swal !== 'undefined') {
          Swal.fire({
            ...
            text: data.message || 'ML models trained successfully',
            timer: 3000
          });
        }
