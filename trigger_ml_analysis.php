...
function trigger_ml_analysis($access_id) {
    ...
    $endpoint = 'http://127.0.0.1' . dirname($_SERVER['SCRIPT_NAME'] ?? '/caps2/ml_model') . '/ml_model/ml_api.php?action=analyze';

    // Fallback if SCRIPT_NAME is not reliable (e.g., CLI contexts)
    if (empty(parse_url($endpoint, PHP_URL_HOST))) {
        $endpoint = 'http://127.0.0.1/caps2/ml_model/ml_api.php?action=analyze';
    }
